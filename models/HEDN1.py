...
class HEDN(nn.Module):
    ...
    def __init__(self, 
        ...
        self.classifier = LabelClassifier(input_dim=64, num_classes=num_classes)

        self.EasyNetwork = ProtoClassifier(
            input_dim=64, 
    ...
    def forward(self, srcs, tgt, src_labels, src_clusters, tgt_cluster):
        ...
        tgt_proto_pred = self.EasyNetwork(
        ...
        src_clu_loss = self.clu_loss(
            self.EasyNetwork.extractor(src_feat_easy),
            src_clusters[easy_idx].squeeze(0),
        )

        tgt_clu_loss = self.clu_loss(
            self.EasyNetwork.extractor(tgt_feat), 
    ...
    def predict_by_easy(self, data):
        ...
        with torch.no_grad():
            ...
            pred = self.EasyNetwork.predict(feat)
        return pred
    ...
    def on_training_start(self, srcs, src_clusters, src_labels):
        ...
        self.EasyNetwork.initialize_cluster_labels(src_labels, src_clusters)
        self.EasyNetwork.initialize_cluster_centers(feats, src_clusters)
    ...
    def get_state(self):
        return {
            ...
            "proto":{
                "src_cluster_labels": self.EasyNetwork.src_cluster_labels.clone().detach(),
                "src_cluster_centers": self.EasyNetwork.src_cluster_centers.clone().detach(),
                "tgt_cluster_centers": self.EasyNetwork.tgt_cluster_centers.clone().detach(),
            }
        }
    
    def load_state(self, state):
        self.load_state_dict(state["model"])
        self.EasyNetwork.src_cluster_labels = state["proto"]["src_cluster_labels"]
        self.EasyNetwork.src_cluster_centers = state["proto"]["src_cluster_centers"]
        self.EasyNetwork.tgt_cluster_centers = state["proto"]["tgt_cluster_centers"]
